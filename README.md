...
## TPU Builders workflow (v6e-8 + Gemma 7B/2B + GCS)

> **Note:** There is **no `ct6e-standard-16t`** in any region. Max v6e GCE VM = **8 chips**. Builders "8×16" = multislice pod (128 chips), not one 16-chip VM. See [docs/tpu_regions.md](docs/tpu_regions.md).

```powershell
.\scripts\list_tpu_capacity.ps1   # scan zones in your project
```
...
.\scripts\provision_tpu.ps1 -ChipCount 8 -VmName ssd-tpu-v6e-8-vm
...
| `provision_tpu.ps1` | Create v6e/v5p VM (`-ChipCount 8` max in us-east5) |
